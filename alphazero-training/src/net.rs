...
// Architecture taken from: https://github.com/BentouAI/AlphaZero-Chain-Reaction/blob/main/model.py
#[derive(Debug)]
pub struct ConvResNetDropout {
...
impl ConvResNetDropout {
    pub fn new(path: &nn::Path, net_config: ConvResNetConfig, options: Options) -> Self {
        ...
        let value_head = Self::build_value_head(path);
        let policy_head = Self::build_policy_head(path);
        Self {
            model,
            policy_head,
            value_head,
    ...
    fn build_model(path: &nn::Path, net_config: ConvResNetConfig) -> nn::SequentialT {
        ...
        // not sure what is 200
        let dropout_blocks = nn::seq_t()
            .add(DropoutBlock::new(
                &(path / "convres_drop1"),
                net_config.hidden_channels * 25,
                200,
            ))
            .add(DropoutBlock::new(&(path / "convres_drop2"), 200, 100));
    ...
    fn build_value_head(path: &nn::Path) -> nn::SequentialT {
        nn::seq_t()
            .add(nn::linear(
                &(path / "vh_linear1"),
                100,
    ...
    fn build_policy_head(path: &nn::Path) -> nn::SequentialT {
        nn::seq_t()
            .add(nn::linear(
                &(path / "ph_linear1"),
                100,
